...
const MOTORSPORT_TEXT_RE = /\b(?:formula\s*1|formula\s*one|formula1|f1|motogp|moto\s*gp|moto\s*\d+|moto\s*e|nascar|indycar|wrc|supercars?|v8sc|wsbk|bsb|british\s*superbikes?|wec|formula\s*e|grand prix|rally|daytona 500)\b/i
...
function isCompetitorVsCompetitorEvent(input = {}) {
  ...
  if (eventClass === 'combat_event' || /^(?:mma|boxing|fighting|kickboxing)$/.test(sport)) return false
...
function classifyByText(text, sport, paired) {
  ...
  if (
    sport === 'motorsport' ||
    /\b(?:formula\s*1|formula\s*one|f1|motogp|moto\s*gp|moto\s*\d+|moto\s*e|nascar|indycar|wrc|supercars?|v8sc|wsbk|bsb|british\s*superbikes?|wec|formula\s*e|grand prix|daytona 500|rally)\b/.test(text)
  ) {
