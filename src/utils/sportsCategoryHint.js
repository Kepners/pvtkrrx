...
function sportKeyFromCategoryName(name) {
  ...
  if (/epl|efl|elc|la liga|serie a|ligue1|bundesliga|champions league|europa league|conference league|international soccer|european soccer|beachsoccer|mls|primeiraportugal|eredivisie|belgianproleague/.test(text)) return 'football'
  ...
  if (/formula1|motogp|nascar|indycar|wrc|automotoracing|motorsport/.test(text)) return 'motorsport'
  if (/darts|premier league darts/.test(text)) return 'darts'
  if (/cycling|tourdefrance|lavuelta|giro/.test(text)) return 'cycling'
  if (/rugby|nrl|gaa/.test(text)) return 'rugby'
  if (/icehockey|nhl|khl|field hockey/.test(text)) return 'hockey'
