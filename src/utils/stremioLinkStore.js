...
class StremioLinkStore {
  constructor(options = {}) {
    this.memory = new Map()
    this.filePath = String(options.filePath || process.env.PVTKRRX_STREMIO_LINK_STORE_FILE || '').trim()
    this.redisUrl = normalizeBaseUrl(
      options.redisUrl ||
      process.env.PVTKRRX_STREMIO_LINK_REDIS_REST_URL ||
      process.env.KV_REST_API_URL ||
      ''
    )
    this.redisToken = String(
      options.redisToken ||
      process.env.PVTKRRX_STREMIO_LINK_REDIS_REST_TOKEN ||
      process.env.KV_REST_API_TOKEN ||
      ''
    ).trim()
  }

  async set(sessionToken, value, ttlSeconds = 1800) {
    const key = this._key(sessionToken)
    if (!key) return false
    const ttl = Math.max(30, Number.parseInt(String(ttlSeconds || 1800), 10) || 1800)
...
module.exports = { StremioLinkStore }
