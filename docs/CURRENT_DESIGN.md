...
- Local `/file` keeps initial buffering states retryable with `425` JSON plus `Retry-After` when the selected file path exists but qBittorrent cannot yet prove a playable start buffer. It does not substitute the waiting-room MP4 for active playback, because that can make Stremio stay on the placeholder instead of retrying the real media URL.
...
- Local `/file` follows the download frontier inside one response while a torrent is still downloading. The `206` advertises the **whole requested range** (`Content-Range`/`Content-Length` run to the end of the range, not to the currently downloaded byte), and the handler keeps feeding that same open response as qBittorrent verifies more pieces. Previously each response ended at the download frontier, so once playback caught up with the download every response was worth only a second or two of video and the player had to reconnect for each one — the "plays ~3 seconds, buffers, plays ~3 seconds" loop reported against long episodes once the ~16MB start buffer was consumed (around 1:47 at typical episode bitrates).
- Follow-streaming behaviour is tunable: `STREAM_FOLLOW_ENABLED` (default `true`, set `false` to restore the old frontier-truncated responses), `STREAM_FOLLOW_CHUNK_BYTES` (`1MB`), `STREAM_FOLLOW_POLL_INTERVAL_MS` (`500`), `STREAM_FOLLOW_STALL_TIMEOUT_MS` (`60000`), `STREAM_FOLLOW_RESUME_AHEAD_BYTES` (`8MB`), `STREAM_FOLLOW_RESUME_MAX_WAIT_MS` (`20000`).
- When a follow response starves, it waits for `STREAM_FOLLOW_RESUME_AHEAD_BYTES` of readable runway (or `STREAM_FOLLOW_RESUME_MAX_WAIT_MS`, whichever comes first) before resuming, so a starved player rebuffers once instead of stuttering forward a piece at a time. If no new pieces land for `STREAM_FOLLOW_STALL_TIMEOUT_MS` the response is destroyed rather than ended short of its advertised `Content-Length`, so the player sees an interrupted transfer and re-requests instead of treating a truncated body as the whole range.
- Completed files still use the plain read-stream path; follow-streaming only applies to in-progress torrents on the built-in local file route.
- Local `/file` prefers a proven-complete copy on the configured `fileServerUrl` over a still-downloading local file. This is how a Google Drive library reaches Stremio: mount/serve Drive over HTTP (rclone), point `fileServerUrl` + `pathMapping` at it, and an in-progress title plays from the finished Drive copy instead of chasing qBittorrent's download frontier. There is no Google Drive API integration and no Drive-specific code — Drive is just a file server URL, which is also why it needs no credentials inside PVTKRRX.
- The remote copy is only used when a `HEAD` proves it is byte-for-byte the same size as the qBittorrent file entry. A half-copied file, a missing file, an unreachable file server, or a server that hides `Content-Length` all fall back to local bytes, so playback never breaks because the mount is behind or offline. Probe results are cached (5 min on a hit, 30s on a miss) so a downloading title does not re-probe on every range request.
- The redirect is skipped when `fileServerAuth` is set on a non-`local` config token, for the same reason tracker playback is: a 302 cannot carry Stremio's `proxyHeaders`, so an authenticated file server is unreachable that way.
- Precedence rule: a **complete** local file still wins (local disk beats the network). The remote copy only takes over while the local copy is incomplete. Tunable via `PVTKRRX_PREFER_REMOTE_COPY` (default `true`), `PVTKRRX_REMOTE_COPY_PROBE_TIMEOUT_MS` (`2500`), `PVTKRRX_REMOTE_COPY_HIT_TTL_MS` (`300000`), `PVTKRRX_REMOTE_COPY_MISS_TTL_MS` (`30000`).

### Google Drive library as a Stremio source

- On Contabo the owner's Drive is an rclone FUSE mount of `gdrive-raw:Plex Media` (`gdrive-media-rclone.service`, read-only at `/opt/stack/gdrive-media/media`, shown to Plex as `/data-gdrive`). `archive-torrents-to-gdrive.sh` copies finished PVTKRRX downloads up there and, once the copy passes a SHA-256 + byte-count check, **deletes the local torrent after 7 days of seeding**. Both live in the separate `contabo-infra-notes` repo — PVTKRRX has no Google Drive API code and holds no Drive credentials.
- That deletion is why `driveLibraryRoot` exists: for anything older than a week the Drive copy is the only copy, so without a library lookup an archived episode disappears from Stremio while still playing fine in Plex.
- Set `driveLibraryRoot` to the mount path and Movie/TV stream requests also resolve against the library, emitting a ready-to-play row alongside tracker sources. Unset (the default) the feature is inert and costs nothing.
- Layout is the archiver's contract, matched by scanning rather than by re-deriving the names: `<root>/Films and TV/TV/<Show>/Season NN/<Show> - SxxEyy[ - detail].<ext>`, `<root>/Films and TV/Films/<file>`. Show folders are matched on `tv_media.py`'s own `identity_key()` normalisation (lowercase, non-alphanumerics collapsed), so punctuation, case, and a `(YYYY)` folder suffix all still match Cinemeta's bare title. Films require an exact year match when one is known, so a remake never resolves to the wrong cut.
- Playback uses the built-in `/file` route's orphan-path branch: the Drive file has no qBittorrent row (the torrent is gone), so the file token carries a synthetic SHA-1-of-path hash instead of an infohash. `/file` no longer fails the whole request when qBittorrent is unreachable and the token path is absolute — a Drive library file needs no qBittorrent at all.
- The lookup is budgeted by `PVTKRRX_STREAM_DRIVE_LIBRARY_TIMEOUT_MS` (default `5000`) and runs alongside the tracker searches, so a cold FUSE listing never delays the stream list. Directory listings are cached for `PVTKRRX_DRIVE_LIBRARY_LISTING_TTL_MS` (default `55000`), just inside the mount's own `--dir-cache-time 1m`, because every uncached `readdir` is a Drive API round trip.
- Hosted relay never emits these rows — the path is local to the host, and the hosted relay must not serve bytes.
...
- If `/playback` times out before it can identify/redirect to a playable file route, it returns retryable `503` JSON plus `Retry-After` so the same token route can be requested again and eventually redirect to real media.
- The bundled waiting-room asset is exposed directly at `/playback/waiting-room.mp4` and `/:config/playback/waiting-room.mp4`. Those asset routes must stay registered before the generic `/:config/playback/:info` token route so the filename is never parsed as a playback token.
- The waiting-room asset is a normal finite `video/mp4` with `Accept-Ranges: bytes`, `Content-Length`, `Content-Range` on partial responses, and `416` on unsatisfied ranges. It does not emulate Stremio's native torrent progress ring, does not proxy provider media, and does not loop by itself. Debrid loader mode can return it from `/playback/debrid`; qBittorrent `/file` and `/playback` still keep not-ready states retryable.
- A real-client test on 2026-05-26 showed static MP4 substitution can leave Stremio on the placeholder instead of playing the real media. Debrid loader mode is therefore explicitly treated as a prepare-screen test mode, not automatic handoff. A future rich loader should use an explicit looping/HLS/retry handoff design.
...
- Stream rows now expose emoji state badges (`⬇️` download-and-play, `⏳` buffering, `✅` downloaded, `📦` extracted), a visible origin badge (`[PC]` for host-PC playback or `[SERVER]` for remote/server playback), and a film-icon container badge (`🎬MKV`, `🎬MP4`, etc.) in the addon `name`. The description now starts with compact facts such as `DL | 1080P | 2.8 GB | MKV | TKR: SportsCult`, `BUFFERING 25% | 1080P | 2.8 GB | MKV | ... | ETA 9m`, or `READY | 1080P | 2.8 GB | MKV | Premiumize`, plus the source serving it. Seeder counts are on their own line. Cached debrid READY rows use the same PVTKRRX logo thumbnail as tracker/qBit rows instead of a text-only provider row.
