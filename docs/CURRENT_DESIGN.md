...
- If a debrid provider is still importing/downloading after the poll window, `/playback/debrid` returns a retryable `503` JSON response with `Retry-After` and progress. It must not substitute the bundled waiting-room MP4 for the active handoff URL because Stremio can treat that placeholder as the selected media and stop re-requesting the real debrid route.
...
- The waiting-room asset is a normal finite `video/mp4` with `Accept-Ranges: bytes`, `Content-Length`, `Content-Range` on partial responses, and `416` on unsatisfied ranges. It is intentionally a direct diagnostic/future-loader asset only: it does not emulate Stremio's native torrent progress ring, does not proxy provider media, does not loop by itself, and must not be returned from active qBittorrent or Debrid wait states.
- A real-client test on 2026-05-26 showed the static MP4 substitution can leave Stremio on the placeholder instead of playing the real media. Any future rich loader must use an explicit looping/HLS/retry handoff design rather than replacing the active playback response with a finite MP4.
