...
- Hosted `/configure` now centers on three main route cards: `PC Local`, `Hybrid Home`, and `Remote Seedbox`.
- Desktop-local `/configure` on the Windows EXE now exposes `PC Local` and `Hybrid Home`; `Remote Seedbox` belongs to the separate server/cloud runtime.
...
| `Hybrid Home` | Other devices on the same Stremio account, including Apple TV | Hosted token manifest generated by configure | Hosted relay 307-redirects to the host's local runtime when the pair is online and the request still matches the home network; otherwise it falls through to hosted/cloud behavior | Host desktop must stay running and heartbeating for the home-network path |
...
- `Hybrid Home` is the main same-account addon for the user's other devices.
- If the host desktop browses the hosted `Hybrid Home` addon and sees `Failed to fetch`, that is a route-choice problem, not proof that the addon server is down.
...
- `Hybrid Home` requests 307-redirect to the host's local runtime before hitting `/file` or `/playback` when the home route is online, so after redirect all local playback capabilities apply.
...
- External `fileServerUrl` is optional. It is required for `Remote Seedbox` completed-file playback on the public hosted relay but not needed for `PC Local`, `Hybrid Home`, or self-hosted server runtimes that can read files directly and serve `/file` / `/playback`.
