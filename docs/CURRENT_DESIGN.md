...
| `Remote Seedbox` | Away-from-home or public playback | Hosted token manifest | External `fileServerUrl` or public qBit URLs for completed files only; no `/playback` queue/buffer on the public hosted relay | Ready-file-first; no tracker queue/buffer unless self-hosted on a playback-capable runtime |
