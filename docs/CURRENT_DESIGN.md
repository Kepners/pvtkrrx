...
- Stream rows now expose emoji state badges (`⬇️` download-and-play, `⏳` buffering, `✅` downloaded, `📦` extracted), a visible origin badge (`[PC]` for host-PC playback or `[SERVER]` for remote/server playback), and a film-icon container badge (`🎬MKV`, `🎬MP4`, etc.) in the addon `name`. The description now starts with compact facts such as `DL | 1080P | 2.8 GB | MKV | TKR: SportsCult`, `BUFFERING 25% | 1080P | 2.8 GB | MKV | ... | ETA 9m`, or `READY | 1080P | 2.8 GB | MP4`, plus the source serving it. Seeder counts are on their own line.
