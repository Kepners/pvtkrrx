...
- If a debrid provider is still importing/downloading after the poll window, `/playback/debrid` now returns the bundled PVTKRRX waiting-room MP4 with `X-PVTKRRX-Waiting-*` and `X-PVTKRRX-Progress` headers instead of only a JSON preparing response. It is a keep-alive/loading surface, not Stremio's native torrent progress ring.
...
- Local `/file` also serves the bundled PVTKRRX waiting-room MP4 for initial buffering states where the selected file path exists but qBittorrent cannot yet prove a playable start buffer. Seek/tail range requests that target unavailable real-media bytes still return retryable range errors rather than substituting the loader clip for the wrong byte range.
