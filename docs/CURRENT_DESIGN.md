...
- Playback priming now explicitly resumes the selected incomplete torrent, moves it to the top of qBittorrent's queue when queueing is enabled, keeps sequential download enabled, and promotes only the chosen playable video/archive files to high file priority while demoting unrelated files.
