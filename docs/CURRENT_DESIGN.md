...
- PVTKRRX still consumes SportsMeta asset routes upstream: `https://sportsmeta.pvtkrrx.cc/asset/{variant}/{sportsmeta:event:...}` for resolved public events, `https://sportsmeta.pvtkrrx.cc/asset/default/{variant}/{sport}?league=...` for unresolved fallbacks, and tokenized member asset routes when a saved Sports Posters member token is present. The PVTKRRX addon surface no longer hands those raw upstream URLs to Stremio directly; it emits `/sports-artwork/id/...png` and `/sports-artwork/default/...png` URLs on the active addon origin, and those proxy routes fetch the selected SportsMeta asset surface and normalize it to the declared PNG dimensions. If canonical poster, landscape, or background artwork upstream is only SVG/generated artwork, the proxy may compose home-vs-away badge artwork from SportsMeta event and badge routes instead of stretching text fallback art. SportsMeta still attaches `X-SportsMeta-Asset-Class`, `X-SportsMeta-Entitlement`, `X-SportsMeta-Source`, and `X-SportsMeta-Gated` response headers on upstream asset routes.
...
- Sports catalog artwork now follows a SportsMeta-owned event contract instead of PVTKRRX-generated league cards:
  ...
  - resolved sports rows use PVTKRRX `/sports-artwork/id/{variant}/{sportsmeta:event:...}.png` URLs backed by the selected SportsMeta canonical/member asset route; if the selected poster, landscape, or background art is generated/SVG and SportsMeta exposes event badge URLs, PVTKRRX composes split-team badge artwork with a centered versus mark in the requested dimensions
...
- PVTKRRX no longer maintains a local TheSportsDB cache or sports poster package path. It does not own the sports image catalogue, but its `/sports-artwork` proxy can compose client-safe fallback poster, landscape, and background PNGs from SportsMeta-owned event and badge routes when the upstream event artwork is only generated/SVG artwork.
...
- Local progressive playback now waits for a safer initial readable buffer. Desktop/local routes still keep qBittorrent first+last piece priority enabled by default while a file is incomplete so Stremio seek/footer probes stay alive, but explicit self-host cloud/server routes now default back to strict head-first sequential download unless `STREAM_PRIORITIZE_LAST_PIECES=true` is set.
...
- For app version `1.1.54`, the current tags are `v1.1.54` and `v1.1.54-selfhost`.
