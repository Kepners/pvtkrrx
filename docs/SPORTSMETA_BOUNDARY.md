...
- the SportsMeta/TheSportsDB coverage policy that decides which sports and competitions get scheduled API enrichment versus deterministic generated/default artwork
...
- SportsMeta may audit tracker category/demand signals, including SportsCult when configured, for enrichment planning. That audit must stay read-only from PVTKRRX's point of view. It cannot create PVTKRRX catalog rows without live Prowlarr/Torznab availability.
...
- SportsMeta now owns a policy-driven scheduled ingest, but live tracker demand audit is still operator-triggered rather than a closed-loop automatic promotion/purge system
