...
PVTKRRX follows the **clockrr pattern** — a hybrid stremio-addon-sdk + Express application deployed as a single hosted Express runtime function. Express handles custom routes for encrypted config tokens, while the SDK router provides standard Stremio protocol compliance.

```
User's Stremio App (any device)
        ↓ HTTPS
PVTKRRX Addon Server (hosted Express runtime, stateless)
...
|-----------|------------|-----------|
| Runtime | Node.js (hosted Express runtime) | Clockrr pattern, free hosting |
...
| Hosting | original zero-cost hosted target | Free, global edge, 10s function timeout |
...
- **Rationale:** hosted Express runtime functions cold-start frequently — in-memory cache won't persist. Redis adds cost and complexity. Cinemeta calls are fast (~100ms). Accept the tradeoff.
...
- **Historical note:** This early hosted-runtime assumption is no longer the live hosted design. Current hosted `Remote Seedbox` on hosted relay is ready-file-first and must not be read as generally queueing and buffering tracker content.
...
Historical note: this example reflects the original hosted-playback plan only. The live hosted `Remote Seedbox` route is ready-file-first and does not generally emit hosted tracker `/playback` URLs.
...
- **Stream response < 10s** — original zero-cost hosted target hard limit
...
|------|--------|------------|
| Hosted timeout too short for slow trackers | Some searches return partial results | Jackett `&timeout=6` param, aggressive AbortController at 7s, return partial via `allSettled()` |
...
| original zero-cost hosted target rate limits (100K/month) | Addon stops working mid-month | Monitor usage, document upgrade path to Pro ($20/month) |
