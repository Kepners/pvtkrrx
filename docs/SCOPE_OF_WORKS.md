...
9. **Hosted deployment** — Single hosted runtime configuration, originally scoped for a zero-cost hosted target
...
**Architecture:** stremio-addon-sdk + Express on hosted Express runtime (clockrr pattern)
...
- `module.exports = app` for the hosted runtime, conditional `app.listen()` for local dev
- Hosted catch-all routing sends application requests to `index.js`
...
|-------|-------------|----------|
| Setup | Project scaffolding, deps, hosted runtime config | 0.5 days |
...
| Integration | End-to-end testing, hosted runtime deployment, Stremio install test | 0.5 days |
| **Total** | | **~4 days** |
...
| Item | Cost | Notes |
|------|------|-------|
| original zero-cost hosted target | $0/month | 100K invocations, 10s timeout |
| Domain (optional) | $0 | Uses `www.pvtkrrx.cc` |
...
**Upgrade path:** paid hosted plan ($20/month) for 60s timeout and 1M invocations if needed.

## 6. Risks & Mitigations

| Risk | Likelihood | Impact | Mitigation |
|------|------------|--------|------------|
| Hosted timeout too short | Medium | High | Aggressive timeouts (7s Jackett, 5s qBit), `Promise.allSettled()` for partial results, Jackett `&timeout=6` param |
...
- original zero-cost hosted target is sufficient for community-scale usage (~100K requests/month)
...
- [ ] Deploys and runs on original zero-cost hosted target (free)
