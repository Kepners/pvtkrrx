...
- Status: deployed for live testing on 2026-05-28. Behavior commit `e5c02611de8fc15906f552e0d88377a9895676d4` is included in GitHub `main` and was rolled to the public Coolify runtime plus native Contabo `/opt/pvtkrrx` runtime. Later docs-only commits may advance the visible image tag without changing this playback/search behavior. This is not yet a packaged Windows EXE/GitHub release revision.
...
- Live proof on 2026-05-28: Coolify deployment row `893` finished for behavior commit `e5c02611de8fc15906f552e0d88377a9895676d4`; `https://www.pvtkrrx.cc/health` returned `HTTP 200`; native `/opt/pvtkrrx/REVISION` contains the same behavior commit and `pvtkrrx.service` is active; server-side `scripts/smoke-prowlarr-search.js`, `scripts/smoke-stream-pipeline.js`, `scripts/smoke-playback-route.js`, and `scripts/smoke-debrid-routing.js` passed.
...
- Live proof: public Coolify row `879` finished for behavior commit `3a210b95ae1d6c10379a072c35b31850cbebbcf7`; public and native Contabo forged `template=broadcast`/`template=glitch` artwork probes returned `X-PVTKRRX-Artwork-Template=ticket-stub` and upstream `template=ticket-stub` despite the public env still carrying `PVTKRRX_SPORTS_POSTER_ADMIN_OVERRIDE=broadcast`. Native `/opt/pvtkrrx` was updated to the same behavior commit and `pvtkrrx.service` is active.
...
- Source/release state: release behavior commit `b2043bc388192eb820fbd3aeeb7b54d3ce3ced38` (`Fix sports poster parser fallbacks`) is the target of GitHub releases `v1.1.74` and `v1.1.74-selfhost`; later `main` commits may be docs-only status updates without changing package version or release assets.
...
- Coolify proof: application id `9` / uuid `w14jewmw5ubscrxh8zzfhq7d` tracks `main` with `application_settings.is_auto_deploy_enabled=true`; deployment queue rows `747` and `748` finished for release behavior commit `b2043bc388192eb820fbd3aeeb7b54d3ce3ced38`, and the public runtime reports package version `1.1.74`.
...
- 2026-04-07 public-site cleanup: the public website stopped advertising `/configure` as a public entry point, `GET /configure` now redirects remote/public requests back to the guide-only site, and runbooks/homepage copy now point users to the Windows host runtime or their own self-host server for real setup. 2026-05-09 manifest rewrite: the Stremio-visible bootstrap addon name is exactly `PVTKRR` (the earlier `PVTKRR Setup` branching was removed and is locked out by CLAUDE.md), and the root manifest is described as a configure-first entry that leads to generated PC Local, LAN Bridge, or Remote Seedbox route manifests.
