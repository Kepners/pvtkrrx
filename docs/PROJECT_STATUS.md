...
- The next self-host release was published as `v1.12.9-selfhost`, keeps the working public PVTKRRX root/configure/bootstrap/manifest URLs explicit at install time using raw stdout, and now auto-wires Prowlarr to qBittorrent during bootstrap even on dirty installs.
