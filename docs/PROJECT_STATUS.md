...
- The next self-host release was published as `v1.12.8-selfhost`, removes the cosmetic `awk` warning from qBittorrent INI writes, and now prints the working public PVTKRRX root/configure/bootstrap/manifest URLs explicitly at install time using raw stdout so they are not redacted.
