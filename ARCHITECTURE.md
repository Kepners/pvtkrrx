...
- PVTKRRX consumes only SportsMeta public/root asset routes upstream and returns client-safe `/sports-artwork/...png` URLs for Stremio clients that cannot reliably render SVG. Sports Posters member tokens and premium poster bytes are not transported through PVTKRRX.
...
7. Sports artwork — canonical posters, backgrounds, logos, and the default-sport fallbacks — still comes from SportsMeta upstream, but the addon now serves client-safe PNG bytes through `/sports-artwork/...png` on the active PVTKRRX origin. PVTKRRX only fetches SportsMeta public/root asset routes; paid Sports Posters member artwork remains on the SportsMeta member addon.
