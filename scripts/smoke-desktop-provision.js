...
assert.match(source, /WINDOWS_STARTUP_RUN_KEY_PATH/, 'desktop runtime should manage a concrete Windows Run-key startup entry')
assert.match(source, /buildWindowsStartupCommand\(/, 'desktop runtime should build an explicit quoted Windows startup command')
assert.match(source, /maybeRepairDesktopStartupRegistration\(/, 'desktop runtime should repair legacy malformed startup registrations')
assert.match(source, /New-ItemProperty -Path/, 'desktop runtime should be able to write the Windows Run-key startup value')
assert.match(source, /Remove-ItemProperty -Path/, 'desktop runtime should be able to remove the Windows Run-key startup value')
